...
Metric3DModel::Metric3DModel(const std::string& modelPath, int imageWidth, int imageHeight)
    : BaseModel(modelPath, imageWidth, imageHeight), env_(ORT_LOGGING_LEVEL_WARNING, "Metric3DInference"), session_(nullptr) {
    Ort::SessionOptions sessionOptions;
    sessionOptions.SetIntraOpNumThreads(2);
    Ort::ThrowOnError(OrtSessionOptionsAppendExecutionProvider_CUDA(sessionOptions, 0));
    session_ = Ort::Session(env_, modelPath.c_str(), sessionOptions);
...
cv::Mat Metric3DModel::preprocess(const cv::Mat& inputImage) {
    cv::Mat imageRgb;
    cv::cvtColor(inputImage, imageRgb, cv::COLOR_BGR2RGB);
...
cv::Mat Metric3DModel::inference(const cv::Mat& preprocessedImage) {
    const char* inputNames[] = {"pixel_values"};
    std::vector<int64_t> inputDims = {1, 3, imageHeight_, imageWidth_};
    Ort::MemoryInfo memoryInfo = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
